...
/*
 * 项目名:    EasyPark
 * 包名       com.hjtech.easypark.utils
 * 文件名:    ToastUtils
 * 创建者:    ZJB
 * 创建时间:  2017/6/20 on 17:54
 * 描述:     TODO
 */
public class ToastUtils {
    ...
    private static int yOffset = (int) (64 * MyApp.getApplication().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) MyApp.getApplication().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(MyApp.getApplication().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(MyApp.getApplication().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(MyApp.getApplication());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(MyApp.getApplication(), spannableString, duration);
            } else {
